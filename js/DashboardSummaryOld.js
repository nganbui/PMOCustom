...
(function(){
	...
	function getSubSites(SubSiteUrl, SubSiteTitle, parentId) {	
		...
		error: function(subsites) {
			console.log('error permission: ' + subsites + SubSiteUrl);
		},
		async: false
		});
	}	
	...
	app.controller("PMOStatusController",['$scope','$http','baseService',function($scope,$http,baseService){				
			...
			if (result!=undefined && result.length>0){
				$.each(result,function(i,item){					
					...
					if (count > 2 && item.Title.toLowerCase().indexOf("(po)") == -1){
						var statusReport = getReportfromProject(item.Url, item.Title);
						//$scope.statusReports.push({SiteURL: item.Url, Sitename: item.Title, StatusReport: statusReport});						
						$scope.statusReports.push(statusReport);	
					}
		...
		function getReportfromProject(siteUrl, siteTitle){							
			//var statusReport = {Id:0,ReportUrl:"",Name:"",Health:"",Budget:"",PercentComplete:"",Week:""};
			//var statusReport = {ProjectURL:siteTitle};
			var statusReport = {};
